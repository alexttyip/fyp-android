...
@Database(entities = [Election::class, KeyPairs::class], version = 1)
@TypeConverters(Converters::class)
abstract class AppDatabase : RoomDatabase() {
    abstract fun electionDao(): ElectionDao
    abstract fun keyPairsDao(): KeyPairsDao
    ...
    private class AppDatabaseCallback(
        ...
        override fun onCreate(db: SupportSQLiteDatabase) {
            ...
            INSTANCE?.let { database ->
                scope.launch {
                    populateDatabase(database.electionDao(), database.keyPairsDao())
                }
            }
        }

        suspend fun populateDatabase(electionDao: ElectionDao, keyPairsDao: KeyPairsDao) {
            // Delete all content here.
            electionDao.deleteAll()
            keyPairsDao.deleteAll()
            ...
            repeat(3) {
                ...
                keyPairsDao.insert(
                    KeyPairs(
                        "UK General Election 203${it}",
                        BigInteger(Random.nextInt(99999999).toString()),
                        BigInteger(Random.nextInt(99999999).toString()),
                        BigInteger(Random.nextInt(99999999).toString()),
                        BigInteger(Random.nextInt(99999999).toString()),
                    )
